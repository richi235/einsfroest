...
set output "throughput_sums.pdf"
...
,     '<grep -v "^\[ CT\]" srtt_min_busy_wait/iperf_tentry.log | tail -n 1'       using  (1.75):(column(4)):xtic("LowRTT") \
,     '<grep -v "^\[ CT\]" MPTCP_in_tree_v5.12/iperf_tentry.log | tail -n 1'      using  (2.5):(column(4)):xtic("MPTCP")   \
,     '<grep -v "^\[ CT\]" afmt_noqueue_busy_wait/iperf_tentry.log | tail -n 1'   using  (3.25):(column(4)):xtic("HTMT")   \
,     "" 					    using  (0.5):2:(0)  \
,     "" 					    using  (3.75):2:(0)  \
