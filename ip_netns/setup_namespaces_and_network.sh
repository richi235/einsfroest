...
tc -netns T_entry qdisc add dev veth11 root handle 20: netem  \
   delay 20ms rate 16mbit

ip netns exec T_entry ip addr add 2.0.0.1/24 dev veth21
ip netns exec T_entry ip link set veth21 up
tc -netns T_entry qdisc add dev veth21 root handle 20: netem  \
   delay 25ms rate 16mbit

ip netns exec T_entry ip addr add 3.0.0.1/24 dev veth31
ip netns exec T_entry ip link set veth31 up
tc -netns T_entry qdisc add dev veth31 root handle 20: netem  \
   delay 25ms rate 32mbit
